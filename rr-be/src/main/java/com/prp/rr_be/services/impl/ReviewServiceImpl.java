...
import com.prp.rr_be.services.ReviewService;
...
public class ReviewServiceImpl
  ...
  public Review createReview(User author, String restaurantId, ReviewCreateUpdateRequest createReview) {
	...
	// Create photos
	List<Photo> photos = createReview
						   .getPhotoIds()
						   .stream()
						   .map(url -> {
  ...
  public Page<Review> listReviews(String restaurantId, Pageable pageable) {
	...
	if (sort.isSorted()) {
	  ...
	  reviews.sort(Comparator
					 .comparing(Review::getDatePosted)
					 .reversed());
	}
  ...
  public Review updateReview(User user, String restaurantId, String reviewId, ReviewCreateUpdateRequest updatedReview) {
	...
	// Update photos
	existingReview.setPhotos(updatedReview
							   .getPhotoIds()
							   .stream()
							   .map(url -> {
								 Photo photo = new Photo();
  ...
  private void updateRestaurantAverageRating(Restaurant restaurant) {
	...
	if (reviews.isEmpty()) {
	  ...
	  float averageRating = (float) reviews
									  .stream()
